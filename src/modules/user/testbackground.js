...
exports.run = (bot, msg, args, mongoose) => {
    if(args && args.length > 0) {
        ...
        sql.exec(async (err, values) => {
            ...
            try {
                const buffer = await profile(msg, user, values[0], args[0].replace(/\D/g,''));
...
async function profile(message, user, userDb, background) {
  try {
    ...
    let bg;
    if(background) {
      bg = await loadImage(`./assets/imgs/backgrounds/${background}.jpg`);
    } else {
